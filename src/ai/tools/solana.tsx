...
const TokenSearchResult = ({ token, className }: { token: any, className?: string }) => {
    return (
        <div className={cn(
                    ...
                    <div className="mt-1 flex items-center gap-2 text-sm text-muted-foreground">
                        ...
                        {token.token_info?.price_info?.total_price && (
                            <>
                                <span>•</span>
                                <span>
                                    Vol: ${(token.token_info.price_info.total_price / 1_000_000_000).toFixed(2)}B
                                </span>
                            </>
                        )}
                    </div>
        ...
        return (
            <div className={cn(
                ...
                <div className="flex items-center gap-3">
                    ...
                    <div className="min-w-0 flex-1">
                        <h3 className="text-base font-medium text-destructive">Transaction Failed</h3>
...
const wallet = {
    resolveWalletAddressFromDomain: {
        ...
        description: 'Resolve a Solana domain name to an address. Useful for getting the address of a wallet from a domain name.',
        isCollapsible: true,
...
const swap = {
    swapTokens: {
        ...
        execute: async ({ inputMint, outputMint, amount, slippageBps = DEFAULT_OPTIONS.SLIPPAGE_BPS }: SwapParams): Promise<SwapResult> => {
            try {
