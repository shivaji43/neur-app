...
function loadFromCache(): NeurUser | null {
    try {
        ...
        if (cached) {
            debugLog("Loading user data from cache", cached, { module: 'useUser', level: 'info' });
            return JSON.parse(cached);
        }
        debugLog("No user data found in cache", null, { module: 'useUser', level: 'info' });
        return null;
    } catch (error) {
        debugLog("Failed to load cached user data", error, { module: 'useUser', level: 'error' });
        return null;
    }
}
...
function saveToCache(data: NeurUser | null) {
    try {
        if (data) {
            ...
            debugLog("User data saved to cache", data, { module: 'useUser', level: 'info' });
        } else {
            localStorage.removeItem("neur-user-data");
            debugLog("User data removed from cache", null, { module: 'useUser', level: 'info' });
        }
    } catch (error) {
        debugLog("Failed to update user cache", error, { module: 'useUser', level: 'error' });
    }
}
 ...
 */
async function fetchNeurUserData(privyUser: PrivyUser): Promise<NeurUser | null> {
    try {
        const response = await getUserData();
        if (response?.data?.success && response?.data?.data) {
            const prismaUser: PrismaUser = response.data.data;
            debugLog("Retrieved PrismaUser data from server", prismaUser, { module: 'useUser', level: 'info' });
            return {
                ...prismaUser,
        ...
        debugLog("Error fetching user data", error, { module: 'useUser', level: 'error' });
        return null;
    }
}
...
export function useUser(): NeurUserInterface {
    ...
    const fetcher = useCallback(async (): Promise<NeurUser | null> => {
        if (!ready || !privyUser) {
            debugLog("Privy not ready or user not logged in", null, { module: 'useUser', level: 'info' });
            return null;
        }

        if (privyUser) {
            debugLog("Fetching NeurUser data from server", null, { module: 'useUser', level: 'info' });
            const neurUser = await fetchNeurUserData(privyUser as PrivyUser);
            debugLog("Merged NeurUser data", neurUser, { module: 'useUser', level: 'info' });
            return neurUser;
        }
        debugLog("No valid NeurUser data retrieved", null, { module: 'useUser', level: 'warn' });
        return null;
    }, [ready, privyUser]);
    ...
    const extendedLogout = useCallback(async () => {
        debugLog("Initiating user logout...", null, { module: 'useUser', level: 'info' });
        ...
        try {
            ...
            saveToCache(null);
            debugLog("User logged out and cache cleared", null, { module: 'useUser', level: 'info' });
            router.replace('/');
        } catch (error) {
            debugLog("Error during logout process", error, { module: 'useUser', level: 'error' });
            router.replace('/');
        }
    }, [privyRest, router]);
