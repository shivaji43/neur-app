...
import { Image as ImageIcon, Loader2, SendHorizontal, X, ChevronDown } from "lucide-react";
...
function MessageAttachments({ attachments, messageId, onPreviewImage }: MessageAttachmentsProps) {
    const validAttachments = attachments.filter((attachment): attachment is Required<Attachment> =>
...
function MessageToolInvocations({ toolInvocations }: { toolInvocations: ToolInvocation[] }) {
    return (
...
function ChatMessage({ message, index, messages, onPreviewImage }: ChatMessageProps) {
    ...
    return (
        <div className="group relative h-16 w-16 shrink-0">
            ...
            <button
                type="button"
                onClick={onRemove}
                className="absolute right-1 top-1 rounded-full bg-background/80 p-1 opacity-0 shadow-sm backdrop-blur-sm transition-all hover:bg-background group-hover:opacity-100"
            >
                <X className="h-3 w-3" />
            </button>
        </div>
            ...
            toolbar={{
                buttons: [
                    <button
                        ...
                        type="button"
                        onClick={onClose}
                        className="absolute top-4 right-4 rounded-full p-2.5 bg-white/10 backdrop-blur-xl hover:bg-white/20 transition-all duration-200"
...
function LoadingMessage() {
    return (
        <div className="flex w-full items-start gap-3">
            ...
            <div className="relative flex max-w-[85%] flex-col gap-2 items-start">
                <div className="relative flex flex-col gap-2 rounded-2xl px-4 py-3 text-sm shadow-sm bg-muted/60">
...
export default function ChatInterface({ id, initialMessages = [] }: { id: string, initialMessages?: Message[] }) {
    const {
        messages,
        input,
        handleSubmit,
            ...
            <div className="relative flex-1 overflow-hidden">
                <ScrollArea className="h-full">
                    <div className="mx-auto w-full max-w-3xl">
                        <div className="space-y-4 pb-36 pt-4 px-4">
                    ...
                    <form onSubmit={handleFormSubmit} className="space-y-4">
                        <div className="relative overflow-hidden rounded-2xl bg-muted">
                            {attachments.length > 0 && (
                                <div className="flex gap-2 overflow-x-auto p-3 bg-muted/50 rounded-t-2xl">
                            ...
                            <div className="absolute bottom-3 right-3 flex items-center gap-2">
                                ...
                                <Button
                                    ...
                                    size="icon"
                                    variant="ghost"
                                    disabled={(!input.trim() && attachments.length === 0) || isLoading || attachments.some(att => att.uploading)}
                                    className="h-8 w-8 hover:bg-muted"
                                >
