...
import { canAffordSubscription, cn, formatDate, getSubPriceFloat } from "@/lib/utils"
...
export function SubscriptionSection({
  ...
  const subscriptionAction = isSubscribed && endDate ? SubscriptionAction.REACTIVATE : isSubscribed ? SubscriptionAction.CANCEL : SubscriptionAction.SUBSCRIBE;
  ...
  return (
    <>
      <AlertDialog open={showConfirmDialog} onOpenChange={setShowConfirmDialog}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>{getDialogHeader(subscriptionAction)}</AlertDialogTitle>
            <AlertDialogDescription>
              {!isSubscribed ? (
                <>
                  You are about to subscribe to Neur for {subscriptionPrice} SOL per month.
                  <br />
                  The SOL balance of the address <b>{wallet.publicKey.slice(0, 4)}...{wallet.publicKey.slice(-4)}</b> will be debited monthly, starting today.
                  <br />
                  <br />
                ...
                <>
                  ...
                  <br />
                  Your subscription will be continued and the next payment will be deducted on <b>{nextPaymentDate?.toLocaleDateString()}</b>
                </>
              ) : (
                <>
                  Are you sure you want to cancel your subscription?
                  <br />
                  Your subscription will remain active until <b>{nextPaymentDate?.toLocaleDateString()}</b>
                </>
              )}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            ...
            <AlertDialogAction
              ...
              disabled={isLoading}
              className={cn(isSubscribed && "bg-destructive hover:bg-destructive/90")}
            >
              {isLoading ? "Processing..." : getConfirmationButtonText(subscriptionAction)}
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
            ...
            <div className="space-y-4">
              <div>
                ...
                <div className="mt-1 flex h-8 items-center">
                  <span
                    className={cn("text-sm font-medium", isSubscribed ? "text-green-500" : "text-muted-foreground")}
                  >
                    {isSubscribed ? "Active" : "Inactive"}
                  </span>
                  {isSubscribed && endDate && (
                    <span className="text-sm text-muted-foreground">&nbsp;ends - {endDate.toLocaleDateString()}</span>
                  )}
                </div>
              </div>
              ...
              <div>
                ...
                <div className="mt-1 flex h-8 items-center justify-between">
                  <span className="text-sm">{subscriptionPrice} SOL per month</span>
                  <Button
                    variant="outline"
                    size="sm"
                ...
                {!isSubscribed && !hasEnoughBalance && (
                  <p className="mt-2 text-xs text-destructive">
                    Insufficient balance. Please fund your wallet with at least {subscriptionPrice} SOL to subscribe.
                  </p>
                )}
              </div>
                          ...
                          <TableBody>
                            {paymentHistory.map((payment) => (
                              <TableRow key={payment.id}>
                                ...
                                <TableCell>
                                  <span
                                    className={cn(
                                      ...
                                      {
                                        "bg-green-50 text-green-700": payment.status === PaymentStatus.SUCCESS,
                                        "bg-red-50 text-red-700": payment.status === PaymentStatus.FAILED,
                                        "bg-yellow-50 text-yellow-700": payment.status === PaymentStatus.PENDING,
                                      },
                                    )}
                                  >
                                ...
                                {payment.status === PaymentStatus.PENDING && <TableCell>Pending</TableCell>}
