...
const checkUserAuth = async () => {
  ...
  const user = await prisma.user.findUnique({ where: { id: userId }, include: { wallets: true, subscription: true } });
  if (user?.earlyAccess) {
    return {
      success: false,
...
export const subscribeUser = actionClient
  ...
  try {
    ...
    const newSubscription = user.subscription || await prisma.subscription.create({
      data: {
        userId: user.id,
        active: false,
    ...
    if (response?.data?.success) {
      console.log(`[subscribeUser] Subscription payment successful for user ${user.id}`);
      ...
      console.error(`[subscribeUser] Subscription payment failed for user ${user.id}:`, response?.data?.error);
...
export const unsubscribeUser = actionClient
  .action(
    async (): Promise<ActionResponse<{ success: boolean }>> => {
  try {
    const authResult = await checkUserAuth();
...
export const reactivateUser = actionClient
  .action(
    async (): Promise<ActionResponse<{ success: boolean }>> => {
  try {
    const authResult = await checkUserAuth();
