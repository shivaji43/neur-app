...
import { createConnection, TransferWithMemoParams } from "@/lib/solana"
...
const parseTransaction = async (txHash: string): Promise<TransferWithMemoParams | null> => {
    try {
        ...
        const transferIx = tx.transaction.message.instructions.find(ix =>
            'program' in ix && ix.program === 'system' &&
            'parsed' in ix && ix.parsed.type === 'transfer'
        )
...
export const checkEAPTransaction = actionClient
    ...
    .action(async ({ parsedInput: { txHash } }): Promise<ActionResponse<{ success: boolean }>> => {
        ...
        if (user?.earlyAccess) {
            return { success: false, error: "User is already in Early Access Program" }
        }
        ...
        try {
            ...
            if (amount !== EAP_PRICE) {
                return { success: false, error: `Invalid amount (expected: ${EAP_PRICE} SOL, received: ${amount} SOL)` }
            }
