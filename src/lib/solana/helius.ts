...
export const searchWalletAssets: (walletAddress: string) => Promise<{ fungibleTokens: FungibleToken[], nonFungibleTokens: NonFungibleToken[] }> = async (ownerAddress: string) => {
    ...
    let fungibleTokens: FungibleToken[] = items.filter((item): item is FungibleToken => item.interface === "FungibleToken" || item.interface === "FungibleAsset");
